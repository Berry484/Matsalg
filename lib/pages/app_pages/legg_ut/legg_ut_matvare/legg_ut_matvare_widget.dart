...
class _LeggUtMatvareWidgetState extends State<LeggUtMatvareWidget>
  ...
  void initState() {
    ...
    if (widget.matinfo != null) {
      ...
      getKommune(matvare.lat ?? 0, matvare.lng ?? 0);
  ...
  Future<void> getUserLocation() async {
    ...
    if (location != const LatLng(0.0, 0.0)) {
      getKommune(location.latitude, location.longitude);
  ...
  Future<void> getKommune(double lat, double lng) async {
    try {
      if (lat == 0 || lng == 0) {
        return;
      }
      ...
      if (token == null) {
        ...
        String? response = await kommuneService.getKommune(token, lat, lng);
